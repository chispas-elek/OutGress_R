...
public class InfoPortal extends Activity {
	...
	/**
	 * 
	 * �ste m�todo ataca un portal, cambiando el poseedor al que lo ha atacado y desbancando al anterior.
	...
	private void atacarPortal(int pIdPortal, int pIDUsuario) {
		...
		try {
			if(cp.get().contains("0")) {
				...
				Toast.makeText(getApplicationContext(), "Ha ocurrido alg�n error, por favor int�ntalo de nuevo", Toast.LENGTH_LONG).show();
			}
		} catch (InterruptedException e) {
			e.printStackTrace();
			Log.e("InterrupException", "Error de interrupci�n a la hora de capturar un portal");
		} catch (ExecutionException e) {
			e.printStackTrace();
			Log.e("ExecuteException", "Error de ejecuci�n a la hora de capturar un portal");
		}
	}
